...
****/
   ...
   UDP_send(s, TicTacToe_QUERY, MAX_SEND_BUFFER, broadcastAddress,remotePort);
	...
	if (status > 0) {
		...
		strcpy_s(port, remotePort);
		strcpy_s(host, broadcastAddress);
/****			
Task 4a: Add code here that will receive a response to the previous broadcast message
****/		
		numBytesRecvd = UDP_recv(s, recvBuffer, MAX_RECV_BUFFER + 1, host, port);
...
****/
			...
			{
				serverArray[numServers].name = std::string(recvBuffer).substr(4);
				serverArray[numServers].host = broadcastAddress;
				serverArray[numServers].port = remotePort;
				numServers++;
