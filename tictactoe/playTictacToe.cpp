...
int playTicTacToe(SOCKET s, std::string serverName, std::string remoteIP, std::string remotePort, int localPlayer)
{
	...
	char yourMoveString[MAX_RECV_BUFFER];
...
****/
			_itoa_s(move, moveString, 2, 10);
			UDP_send(s, moveString, strlen(moveString),(char*)serverName.c_str(),(char*)remotePort.c_str());
...
Task 2: (i) Insert code inside this IF statement that will accept a null-terminated C-string from your
		...
		(ii) call a function that will update the game board (see above) using your opponent's move(updateBorad(opponent), and
		(iii) call a function that will display the updated board on your screen.

****/
				UDP_recv(s, yourMoveString, strlen(yourMoveString), (char*)serverName.c_str(), (char*)remotePort.c_str());
				updateBoard(board, atoi(yourMoveString), opponent);
				displayBoard(board);
